...
class HtmlEscaper
{
    ...
    {
        return $stringer
            ->callback(fn(Stringer $stringer): string => htmlspecialchars($stringer->toString(), $flag, $encoding));
    }
    ...
    public static function url(Stringable $stringer, array $protocol = ['http', 'https', 'ftp', 'mailto']): Stringable
    {
        ...
        return $stringer
            ->trim()
            ->replace([' ', "\t", "\n", "\r"], '')
            ->callback(fn(Stringer $stringer): Stringer => new Stringer(html_entity_decode($stringer, ENT_QUOTES | ENT_HTML5, 'UTF-8')))
            ->replace('#(?<!:)//+#', '/');
    }
}