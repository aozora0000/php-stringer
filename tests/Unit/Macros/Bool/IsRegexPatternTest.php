...
class IsRegexPatternTest extends TestCase
{
    ...
    #[Test]
    public function デリミタが不正な場合は偽を返す()
    {
        $stringable = $this->createMock(Stringable::class);
        $stringable->method('__call')->willReturnCallback(fn () => '/abc[');
    ...
    #[Test]
    public function デリミタが1文字未満の場合は偽を返す()
    {
        $stringable = $this->createMock(Stringable::class);
        $stringable->method('__call')->willReturnCallback(fn () => 'ab');
    ...
    #[Test]
    public function 修飾子付き正規表現は真を返す()
    {
        $stringable = $this->createMock(Stringable::class);
        $stringable->method('__call')->willReturnCallback(fn () => '/abc/i');
    ...
    #[Test]
    public function 不正な正規表現構文の場合は偽を返す()
    {
        $stringable = $this->createMock(Stringable::class);
        $stringable->method('__call')->willReturnCallback(fn() => '/[abc');
